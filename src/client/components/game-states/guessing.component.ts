  ...
  <div style="display: flex; flex-direction: column; text-align: center;">
    ...
    <ul class="mb-2">
      <li class="highlight" v-for="word in Object.keys(userWords)">
        {{ word.startsWith('<<') ? userWords[word].join(", ") + $t('guessingHintsSameWord') : word }}
      </li>
    </ul>

    <div v-if="isActivePlayer">
      <div class="mb-2">{{ $t('guessingYouHave') }} <span class="highlight">{{guessesLeft}}</span> {{ $t('guessingGuessesLeft') }}</div>
    ...
    <div v-if="!isActivePlayer" class="mb-2">
      <div><span class="highlight">{{activePlayer}}</span> {{ $t('guessingHas') }} <span class="highlight">{{guessesLeft}}</span> {{ $t('guessingGuessesLeft') }}</div>
    </div>
